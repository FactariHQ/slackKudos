...
var CONFIG_DEFAULTS = {
  ...
  ASYNC_COMMANDS: { value: true, notes: 'TRUE runs slash commands on a one-off trigger and sends the answer to response_url, instead of making Slack wait. A give takes 2.5-4s of real work and Slack gives up at 3s, so this is what stops "operation_timeout". FALSE answers inline, the old way.' },
